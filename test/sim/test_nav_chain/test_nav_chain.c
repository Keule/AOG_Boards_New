 ...
 *   Fake TCP RX           → ntrip_client → RTCM Buffer
...
static uint8_t fake_tcp_rx_storage[512];
static byte_ring_buffer_t fake_tcp_rx;
...
void setUp(void)
{
    ...
    memset(fake_tcp_rx_storage, 0, sizeof(fake_tcp_rx_storage));
    byte_ring_buffer_init(&fake_tcp_rx, fake_tcp_rx_storage, sizeof(fake_tcp_rx_storage));
    ntrip_client_set_tcp_source(&ntrip, &fake_tcp_rx);
...
void test_both_gnss_instances_independent(void)
{
    ...
    /* service_step reads in 64-byte chunks — call twice for full sentences */
    gnss_um980_service_step((runtime_component_t*)&primary_gnss,   1000000);
    gnss_um980_service_step((runtime_component_t*)&primary_gnss,   2000000);
    gnss_um980_service_step((runtime_component_t*)&secondary_gnss, 1000000);
    gnss_um980_service_step((runtime_component_t*)&secondary_gnss, 2000000);
...
/* ================================================================
 * Teil 2 – Fake TCP RX → NTRIP → CONNECTED → RTCM buffer
 * ================================================================ */

/**
 * Helper: fast-forward ntrip to CONNECTED state via service_step calls.
 * Returns the timestamp after connection is established.
 */
static uint64_t fast_forward_ntrip_to_connected(void)
{
    ntrip_client_start(&ntrip);
    TEST_ASSERT_EQUAL(NTRIP_STATE_CONNECTING, ntrip_client_get_state(&ntrip));

    /* CONNECTING → AUTHENTICATING (after 1s) */
    ntrip_client_service_step((runtime_component_t*)&ntrip, 1500000);
    TEST_ASSERT_EQUAL(NTRIP_STATE_AUTHENTICATING, ntrip_client_get_state(&ntrip));

    /* AUTHENTICATING → CONNECTED (after another 1s) */
    ntrip_client_service_step((runtime_component_t*)&ntrip, 3000000);
    TEST_ASSERT_EQUAL(NTRIP_STATE_CONNECTED, ntrip_client_get_state(&ntrip));
...
{
    fast_forward_ntrip_to_connected();

    /* Push RTCM bytes into fake TCP source */
    byte_ring_buffer_write(&fake_tcp_rx, RTCM_BYTES, RTCM_BYTE_COUNT);

    /* service_step should forward TCP → RTCM buffer */
...
void test_rtcm_router_distributes_to_both_uart_tx(void)
{
    ...
    /* Push NTRIP through the full chain to CONNECTED */
    fast_forward_ntrip_to_connected();

    /* Push RTCM data into TCP → NTRIP forwards to rtcm_buffer */
    byte_ring_buffer_write(&fake_tcp_rx, RTCM_BYTES, RTCM_BYTE_COUNT);
    ntrip_client_service_step((runtime_component_t*)&ntrip, 3100000);
