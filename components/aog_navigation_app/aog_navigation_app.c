...
/* ---- Constants ----
 * NAV-FIX-001-R2 AP-A: PGN214 output at 100 Hz (10 ms / 100 Hz).
 * Previous: 50 ms / 20 Hz — REMOVED, no longer active. */
...
void aog_nav_app_fast_input(runtime_component_t* comp, const fast_cycle_context_t* ctx)
{
    ...
    /* Read incoming AOG frames (Hello + Scan Request detection)
...
/* ==================================================================
 * FAST_PROCESS: Core AOG-NAV business logic.
 *
 * - Reads GNSS/Heading snapshots
 * - Performs output gating (8-state model)
 * - Builds PGN 214 frames
 * - Writes to TX buffer
 *
 * NAV-FIX-001-R2 AP-B: This is the PRIMARY production path.
 * service_step is NO LONGER used for AOG-NAV business logic.
 * Called from task_fast on Core 1 at 100 Hz.
 * ================================================================== */
...
void aog_nav_app_fast_process(runtime_component_t* comp, const fast_cycle_context_t* ctx)
{
    ...
    app->cycle_count++;
    uint32_t now_ms = (uint32_t)(ctx->timestamp_us / 1000);

    /* ==================================================================
     * HELLO RESPONSE (PGN 254) — only on PGN 253 request
    ...
    /* ==================================================================
     * SCAN REPLY (PGN 203) — only on PGN 202 request
     ...
     * No separate interval gating — the fast path IS the 100 Hz clock.
     * ================================================================== */
    ...
    /* ==================================================================
     * OUTPUT GATING — strict per NAV-AOG-001-FINAL
     *
...
/* ==================================================================
 * FAST_OUTPUT: Placeholder — no additional output phase needed.
 * AOG-NAV writes directly to TX buffer in fast_process.
 * ================================================================== */

void aog_nav_app_fast_output(runtime_component_t* comp, const fast_cycle_context_t* ctx)
{
    /* No-op: PGN214 frames are already pushed in fast_process.
     * This hook exists for structural consistency in the 3-phase
     * fast path model. If future needs arise (e.g. TX flush),
...
void aog_nav_app_init(aog_nav_app_t* app)
{
    ...
    app->component.service_step = NULL;
...
/* ==================================================================
 * LEGACY service_step — NOT USED in production.
 *
 * NAV-FIX-001-R2 AP-B: This function is retained ONLY as a
 * backward-compatible wrapper for host tests that have not yet
 * been migrated to the fast_process API.
 *
 * It does NOT run in the Core 0 service loop (service_step = NULL).
 * It simply delegates to fast_process with a synthetic context.
 * ================================================================== */

void aog_nav_app_service_step(runtime_component_t* comp, uint64_t timestamp_us)
{
    fast_cycle_context_t synthetic_ctx;
