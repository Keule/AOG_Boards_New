...
void ntrip_client_service_step(runtime_component_t* comp, uint64_t timestamp_us)
{
     ...
     * Pretend the transition happened before the timeout period
     * so the first service_step will trigger the state transition. */
    if (client->last_state_change_us == 0 && timestamp_us > 0) {
        client->last_state_change_us = (timestamp_us > NTRIP_SKELETON_CONNECTING_TIMEOUT_US)
            ? (timestamp_us - NTRIP_SKELETON_CONNECTING_TIMEOUT_US)
            : 1;
    }
    ...
    case NTRIP_STATE_ERROR:
        ...
        if (elapsed >= NTRIP_SKELETON_AUTHENTICATING_TIMEOUT_US) {
            ...
            /* RECONNECT is a transient state — immediately progress to CONNECTING */
